...
_test('Phone search', assert =>{
  ...
  client.phoneSearch({
    ...
    assert.equals(response.jsonBody.businesses[0].alias, 'gary-danko-san-francisco');
    assert.end();
  }).catch(e =>{
    assert.fail(e);
  });
...
_test('Event search', assert =>{
  ...
  client.eventSearch({
    ...
    assert.true(response.jsonBody.events);
    assert.end();
  }).catch(e =>{
    assert.fail(e);
  });
...
_test('Event Lookup', assert =>{
  const client = _yelp.client(apiKey);

  client.eventLookup("oakland-saucy-oakland-restaurant-pop-up", {
    locale: "en_US"
  }).then(response => {
    assert.equals(response.jsonBody.id, "oakland-saucy-oakland-restaurant-pop-up");
    assert.end();
  }).catch(e => {
    assert.fail(e);
  });
